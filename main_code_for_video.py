...
#        
        ...
        for j in range(num_particle):      
            ...
            if particle_List[j].PN > 0:
                ...
                cv2.rectangle(img_gray_draw,(x_fl-50,y_fl-65),(x_fl+50,y_fl+65) , 255, 2)

            
                if particle_List[j].type=='unknown' and sub_image.size==80*160:
                    ret,thresh1 = cv2.threshold(sub_image,9,255,cv2.THRESH_BINARY)
                    temp = cv2.bitwise_and(sub_image,sub_image,mask = thresh1)
                    average=temp.sum()/80/160
                    particle_List[j].fl_predict.append(average)
            
        #cut gray_draw and resize
        ...
        for j in range(num_particle): 
            if particle_List[j].PN>0:
                if particle_List[j].type=='unknown':
                    cv2.putText(img_gray_draw, str(particle_List[j].PN), (particle_List[j].position_x[-1]-60,particle_List[j].position_y[-1]-65), font, 0.7, (255), 1, cv2.LINE_AA)  
                else:
                    cv2.putText(img_gray_draw, str(particle_List[j].type), (particle_List[j].position_x[-1]-60,particle_List[j].position_y[-1]-65), font, 0.7, (255), 1, cv2.LINE_AA)
        #add camera note
        cv2.putText(img_gray_draw, 'Fluorescence camera is active', (10,900), font, 1.5, (255), 2, cv2.LINE_AA)
...
#            cv2.putText(img_gray_draw, str(j),tuple([particle_List[j].position_x[-1],particle_List[j].position_y[-1]]), font, 1, (255), 1, cv2.LINE_AA)
    ...
    #           set number detected
        ...
        num_particle=len(particle_List)
        for j in range(num_particle):
            #adjust life accordind to speed y
            if particle_List[point_p].speed[1]<-1:
                particle_List[point_p].appear -= 4
            if particle_List[point_p].appear <= 0:
                #if only one object left,do not delete it.
                if len(particle_List)>1:
                    del particle_List[point_p]
    ...
    #            if hog_fake[i_det]==0:
        ...
        for j in range(num_particle):        
            if particle_List[j].PN > 0:
                ...
                #predict type of this particles
                if particle_List[j].type=='unknown' and sub_image.size==96*96:
                    if len(particle_List[j].fl_predict)>3:
                        #get average of fl_predict
                        fl_indicator = sum(particle_List[j].fl_predict) / len(particle_List[j].fl_predict)
                        
                        #get average of lensf predict
                        if len(particle_List[j].lenf_predict)>1:
                            lensf_indicator=np.zeros((3))
                            for temp in particle_List[j].lenf_predict:
                                lensf_indicator=lensf_indicator+temp
                            lensf_indicator = lensf_indicator / len(particle_List[j].lenf_predict)
                        #revise lensf_indicator using fl_indicator
                        if 3>fl_indicator>0.3:
                            lensf_indicator[0]+=0.9
                        if 3>fl_indicator:
                            lensf_indicator[1]+=0.3
                        if fl_indicator<0.1:
                            lensf_indicator[2]+=1
                            
                        #assign type
                        particle_type = np.where(lensf_indicator == np.amax(lensf_indicator))
                        if particle_type[0][0]==0:
                            particle_List[j].type='Cyanobacteria'
                            ca_count+=1
                        elif particle_type[0][0]==1:
                            particle_List[j].type='Euglena'
                            eu_count+=1
                        elif particle_type[0][0]==2:
                            particle_List[j].type='Plastic bead'
                            ps_count+=1
                            
                    
                    
                    ########## get input of lensf_model
                    #subimage is big enough, cut image size to 64x64
                    img_temp=sub_image[16:-16,16:-16]
                    ...
                    temp_input=np.zeros((145))
                    temp_input[0]=contrast
                    temp_input[1:]=descriptor
                    temp_input=np.reshape(temp_input,(1,145))
                    PN_lensf_predictions = model_lensf.predict_proba(temp_input) 
                    #update particle
                    particle_List[j].lenf_predict.append (PN_lensf_predictions[0])
        ...
        cv2.putText(img_gray_draw, 'Holographic camera is active', (10,900), font, 1.5, (255), 2, cv2.LINE_AA)
        

                
    ###save images and summary
    #put summary
    cv2.putText(img_gray_draw, 'Plastic beads:'+str(ps_count), (10,40), font, 1, (255), 1, cv2.LINE_AA)
    cv2.putText(img_gray_draw, 'Cyanobacteria:'+str(ca_count), (10,140), font, 1, (255), 1, cv2.LINE_AA)
    cv2.putText(img_gray_draw, 'Euglena:'+str(eu_count), (10,240), font, 1, (255), 1, cv2.LINE_AA)
