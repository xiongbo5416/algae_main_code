...
def point2bbox_image(image,x,y,WIDE_WINDOW,HEIGHT_WINDOW):
    ...
    image_out=np.zeros((WIDE_WINDOW,HEIGHT_WINDOW),int)
...
df = pd.DataFrame(data, columns = ['frame', 'PN', 'speed', 'location_x','location_y', 'lensf', 'lensf_subimage','fl_subimage', 'case','period_n']) 
    ...
    if f[-5]=='f':
        ...
        for j in range(num_particle):        
            if particle_List[j].PN > 0:
                ...
                sub_image = point2bbox_image(img_gray,particle_List[j].position_x[-1],particle_List[j].position_y[-1] + 50, 40, 100)
                ...
                cv2.circle(img_gray_draw,(particle_List[j].position_x[-1],particle_List[j].position_y[-1]+50), 20, 128, 3)
    ...
    #            if hog_fake[i_det]==0:
        ...
        for j in range(num_particle):        
            if particle_List[j].PN > 0:
                ...
                #write data into pandas dataframe
                ##pd.DataFrame['frame', 'PN', 'speed', 'location_x','location_y', 'lensf', 'lensf_subimage', 'case']) 
                df=df.append({'frame' : frame_num ,'PN' :particle_List[j].PN, 'speed':speed_temp,'location_x':particle_List[j].position_x[-1], 'location_y':particle_List[j].position_y[-1], 'lensf': 1,'lensf_subimage': sub_image , 'case': particle_List[j].case, 'period_n': period_n}, ignore_index = True) 
