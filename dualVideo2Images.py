...
threshold_ex = 3 #set it to 0 when want to decide it 
current_frame=0
period_num=0
current_frame_fl=0
    ...
    if len(bgd_name) == N_MOVING:        
        ...
        if len(foldername)>0:
            for i in range(N_MOVING):
                ...
                name= foldername +'/' +str(bgd_name[i]+1000) +'.bmp'
                cv2.imwrite(name,gray_dif)
        #reset
        bgd_name=[]
        i_MOVING=0
...
if len(videoname2) >0:
    ...
    #threshold_ex = img_cut.sum()/(height * width)/3
    print(bbox)
    ...
    while current_frame_fl<current_frame:
        ...
        gray_0 = np.array(frame[:, :, 1])
        fl_float=gray_0.astype(float) 
   
        fl_profile=sum(fl_float.T)
        fl_profile=fl_profile/bbox[2]
        #print(min(fl_profile))
        
        
        #blue light is off
        if min(fl_profile)< threshold_ex:
            excitation_st=0
            
        else: #blue light is on for hole frame
...
#    
    ...
    #           cv2.putText(fg_gray_reg,'blue laser on',(50,150), font, 4, (255), 4, cv2.LINE_AA)
           ...
           #save frames
           if len(foldername)>0:
               name= foldername +'/' + str(lensfr_period[period_num-1]+excitation_st-1+NUM_COMP) +'f.bmp'
               gray_0=cv2.multiply(gray_0,FL_EH)#enhacne contrast of gray images
               cv2.imwrite(name,gray_0) 
